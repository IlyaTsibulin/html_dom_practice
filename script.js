...
//     row.innerHTML = `
//     <td>${cats[key][i].age}</td>
//     <td>${cats[key][i].rate}</td>
//     <td>${cats[key][i].favourite}</td>
//     <td>${cats[key][i].description}</td>
//     `
// }
...
document.querySelector('.content').innerHTML = `<table class='cats'></table>`
for (key of cats) {
    let name = document.createElement('tr')
    name.innerHTML = `<td colspan='2'>${key.name}</td>`
    document.querySelector('.cats').appendChild(name)

    let img_link = document.createElement('tr')
    img_link.innerHTML = `<td colspan='2'><img src=${key.img_link}></td>`
    document.querySelector('.cats').appendChild(img_link)

    let age = document.createElement('tr')
    age.innerHTML = `<td colspan='3'>Возраст: ${key.age} год/года/лет</td>`
    document.querySelector('.cats').appendChild(age)

    let rate = document.createElement('tr')
    rate.innerHTML = `<td colspan='3'>Оценка: ${key.rate}/10</td>`
    document.querySelector('.cats').appendChild(rate)

    let favourite = document.createElement('tr')
    if (key.favourite) {
        key.favourite = 'Да'
    } else {
        key.favourite = 'Нет'
    }
    favourite.innerHTML = `<td colspan='3'>В избранном: ${key.favourite}</td>`
    document.querySelector('.cats').appendChild(favourite)

    let description = document.createElement('tr')
    description.innerHTML = `<td colspan='3'>Описание: ${key.description}</td>`
    document.querySelector('.cats').appendChild(description)
}